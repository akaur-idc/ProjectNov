...
import objectRepository.Slider;
...
public class AssessmentSubmission {
	...
	public void RunAssessment(WebDriver Project, String url, ExcelData exlData, int row)
		...
		// RADIO ON PAGE 1
			//+(Project.findElements(By.cssSelector("exportOuterCircle"))).size());
		...
		//  radio_Options = radioOption.getRadioCount(exlData, toolPage);
		  radio_Options = radioOption.fetchRadioCount(Project, el.Radio1);
			
		  radioOption.getRadioOption(Project, toolPage, radio_Options, exlData, row);
		 
		Project.manage().timeouts().implicitlyWait(80, TimeUnit.SECONDS);
		
		JavascriptExecutor js = (JavascriptExecutor)Project;
		//js.executeScript("document.getElementById('Q1-QI1-AC1').click();");
		//#section1page1Form > div > fieldset > section > div > div > section > div.row > div
		//
		//System.out.println("======RADIO COUNT PG1 =========" +Project.findElements(By.xpath("//input[@name='Q1-QI1']")).size());
		...
		// to get radio options count
		//radio_Options = radioOption.getRadioCount(exlData, toolPage);
		radio_Options = radioOption.fetchRadioCount(Project, el.Radio2);
			
		radioOption.getRadioOption(Project, toolPage, radio_Options, exlData, row);

		// Project.manage().timeouts().implicitlyWait(2000, TimeUnit.SECONDS);
		Thread.sleep(2000);
		//System.out.println("======RADIO COUNT PG2 =========" +Project.findElements(By.xpath("//input[@name='Q2-QI2']")).size());
		...
		checkBox_Options = chbox.fetchChekboxCount(Project,el.Checkbox);
			
		
		//System.out.println("======CHECKBOX COUNT PG5 =========" +Project.findElements(By.xpath("//input[@name='Q7-QI9']")).size());
		...
		// SLIDER
		//slider.maxSlider(Project);
		slider.getSlider(Project, exlData, row);
